...
cmd({
  pattern: "setalive",
  desc: "Set the alive2 image and message. Usage: ${settings.PREFIX}setalive <image/video_url> | <caption>",
  category: "main",
  filename: __filename
  ...
  if (!input.includes('|')) {
    return reply("❌ Usage: ${settings.PREFIX}setalive <image/video_url> | <caption>\nUse .tourl to convert image to URL\nUse {runtime} to show bot uptime.");
  }
  ...
  if (!url.trim() || !caption) {
    return reply("❌ Both URL and caption are required.\nUsage: ${settings.PREFIX}setalive <image/video_url> | <caption>");
  }

  setAlive2Settings(url.trim(), caption);
  reply("✅ Alive2 message and image saved! Use ${settings.PREFIX} alive2 to test.");
});
